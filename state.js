// UI-only state for the soundbox editor (plans/soundbox-revamp.md Phase 3
// Stage B). The song/instrument data itself is a plain, JSON-serializable
...
export const state = {
  ...
  editMode: 'pattern',
  playing: false,
  // Callbacks main.js installs at boot so panels/tracker.js (Stage D) can
  ...
  notify: null,
  requestStop: null,
  // Set to panels/keyboard.js's previewNote (Stage E) at boot. Lets
  ...
  previewNote: null,
  // Set to panels/keyboard.js's highlightPlaybackNotes (Stage E.2) at boot.
  ...
  highlightNotes: null,
  // Set to panels/keyboard.js's shadowNotes (Stage E.14) at boot. Lets the
  ...
  // --- note-input preferences (plans/soundbox-scale-chord-input.md).
  // Deliberately *not* song data: which scale you play in and how many rows
  ...
  // edit step, added in Stage E.14 because the pie menu forced the question
  // "where does the cursor go after a pie commit" and a number answers it
  // better than a yes/no: 1 = the next row (what enterNoteAtCursor always
  // hardcoded), 0 = stay put, 4 = a beat at a time. Applies to every note
  // entry path -- typed, on-screen piano, pie -- so they can't drift apart.
  editStep: 1,
  // Which characters the physical keyboard prints, for the on-screen piano's
  // key hints only -- note entry stays positional (Stage E.15, see layouts.js).
