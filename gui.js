...
// The song/instrument data model (binary encode/decode, JS export) has
// moved to engine.js, a real DOM-free ES module -- index.html loads it via
// a small <script type="module"> bridge and exposes it as window.SBEngine,
// since this file is still a plain classic script (plans/soundbox-revamp.md
// Phase 3 Stage A).
...
var CGUI = function()
{
  ...
  // Misc constants (shared with engine.js, single source of truth there)
  var MAX_SONG_ROWS = SBEngine.MAX_SONG_ROWS,
      MAX_PATTERNS = SBEngine.MAX_PATTERNS,
      MAX_CHANNELS = SBEngine.MAX_CHANNELS;
  ...
  // Song data model (binary import/export, JS export) now lives in
  // engine.js (plans/soundbox-revamp.md Phase 3 Stage A) -- these are
  // aliases into that module so the rest of this file is unchanged.
  var calcSamplesPerRow = SBEngine.calcSamplesPerRow;

  var getBPM = function () {
    return Math.round((60 * 44100 / 4) / mSong.rowLen);
  };

  // Instrument property indices (shared with engine.js, single source of
  // truth there)
  var OSC1_WAVEFORM = SBEngine.OSC1_WAVEFORM,
      OSC1_VOL = SBEngine.OSC1_VOL,
      OSC1_SEMI = SBEngine.OSC1_SEMI,
      OSC1_XENV = SBEngine.OSC1_XENV,

      OSC2_WAVEFORM = SBEngine.OSC2_WAVEFORM,
      OSC2_VOL = SBEngine.OSC2_VOL,
      OSC2_SEMI = SBEngine.OSC2_SEMI,
      OSC2_DETUNE = SBEngine.OSC2_DETUNE,
      OSC2_XENV = SBEngine.OSC2_XENV,

      NOISE_VOL = SBEngine.NOISE_VOL,

      ENV_ATTACK = SBEngine.ENV_ATTACK,
      ENV_SUSTAIN = SBEngine.ENV_SUSTAIN,
      ENV_RELEASE = SBEngine.ENV_RELEASE,
      ENV_EXP_DECAY = SBEngine.ENV_EXP_DECAY,

      ARP_CHORD = SBEngine.ARP_CHORD,
      ARP_SPEED = SBEngine.ARP_SPEED,

      LFO_WAVEFORM = SBEngine.LFO_WAVEFORM,
      LFO_AMT = SBEngine.LFO_AMT,
      LFO_FREQ = SBEngine.LFO_FREQ,
      LFO_FX_FREQ = SBEngine.LFO_FX_FREQ,

      FX_FILTER = SBEngine.FX_FILTER,
      FX_FREQ = SBEngine.FX_FREQ,
      FX_RESONANCE = SBEngine.FX_RESONANCE,
      FX_DIST = SBEngine.FX_DIST,
      FX_DRIVE = SBEngine.FX_DRIVE,
      FX_PAN_AMT = SBEngine.FX_PAN_AMT,
      FX_PAN_FREQ = SBEngine.FX_PAN_FREQ,
      FX_DELAY_AMT = SBEngine.FX_DELAY_AMT,
      FX_DELAY_TIME = SBEngine.FX_DELAY_TIME;

  var makeNewSong = SBEngine.makeNewSong;
  var songToBin = SBEngine.songToBin;
  var soundboxBinToSong = SBEngine.soundboxBinToSong;
  var sonantBinToSong = SBEngine.sonantBinToSong;
  var instrumentToBin = SBEngine.instrumentToBin;
  var binToInstrument = SBEngine.binToInstrument;
  var binToSong = SBEngine.binToSong;
  var songToJS = SBEngine.songToJS;
  ...
  var showAboutDialog = function () {
    ...
    o.innerHTML = "Licensed under the <a href=\"gpl.txt\" target=\"_blank\" rel=\"noopener noreferrer\">GPL v3</a> license " +
                  "(get the <a href=\"https://gitlab.com/mbitsnbites/soundbox\" target=\"_blank\" rel=\"noopener noreferrer\">source</a>).";
