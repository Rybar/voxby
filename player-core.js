...
// The song renderer, lifted out of player-worker.js so it can run on the main
// thread as well as inside the worker.
//
// Nothing in here changed but one line: the constructor used to call the
// worker global postMessage() directly to report progress, which is the only
// thing in it that could not run anywhere else. That is now an `onProgress`
// hook the caller sets -- player-worker.js sets it to the postMessage it
// always did, and core/oneshot.js (which renders one short sound synchronously
// and wants no progress at all) leaves it unset.
//
// Marcus Geelnard's code, under the GPL v3 that came with SoundBox; the
// licence block above is his and travels with it.

"use strict";

export const CPlayerWorker = function () {
